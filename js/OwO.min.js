                ...
                value: function(e) {
                    ...
                    for (var a = '\n            <div class="OwO-logo"><span>' + e.logo + '</span></div>\n            <div class="OwO-body" style="width: ' + e.width + '">',
                        ...
                        for (var n = this.odata[this.packages[s]].container, i = 0; i < n.length; i++){
                            var type = this.odata[this.packages[s]].type;
                            if (type == "image") {
                                a += '\n                    <li class="OwO-item" data-id="'+ n[i].data +'" title="' + n[i].text + '">' + n[i].icon + "</li>";
                            }else{
                                a += '\n                    <li class="OwO-item" data-id="not-given" title="' + n[i].text + '">' + n[i].icon + "</li>";
                            }
                        }
                        a += "\n                </ul>"
                    }
                    ...
                    a += "\n                    </ul>\n                </div>\n            </div>\n            ",
                        ...
                        this.container.getElementsByClassName("OwO-body")[0].addEventListener("click",
                            function(e) {
                                ...
                                if (target) {
                                    const cursorPos = t.area.selectionEnd;
                                    let areaValue = t.area.value;
                                    if (target.dataset.id == "not-given") {
                                        t.area.value = areaValue.slice(0, cursorPos) + target.innerHTML + areaValue.slice(cursorPos);
                                    } else {
                                        t.area.value = areaValue.slice(0, cursorPos) + target.dataset.id + areaValue.slice(cursorPos);
                                    }
                                    t.area.focus();
                                    t.toggle();
                                }
